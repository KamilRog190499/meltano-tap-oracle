...
Note: This tap now uses Oracle's `python-oracledb` driver. Set `ora_python_driver_type` to `thin` (default) or `thick` depending on your environment.

```shell
# For more details on the Oracle oracledb library refer to
...
* `"ora_python_driver_type": "thick|thin"` - Selects python-oracledb mode. Default is thin.
...
A setting which configures the `python-oracledb` connection mode. You can run it in either `thick` or `thin` mode. Under thin mode you can pass specific driver settings like `https_proxy` if required via config settings.
